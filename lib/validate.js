...
const util = {
  ...
  validateFilter: function (req, res, next) {
    ...
    if (material) {
      ...
      if (type) {
        if (type.indexOf('-') !== -1 && !validator.isIn(type.substr(0, type.indexOf('-')), TYPES)) {
          return next(new NotFoundError('Incorrect type'))
        } else if (type.indexOf('-') === -1 && !validator.isIn(type, TYPES)) {
          return next(new NotFoundError('Incorrect type'))
        }
  ...
  validateListing: function (req, res, next) {
    ...
    if (!validator.isLength(newListing.description, { min: 5, max: 200 })) {
      return next(new BadRequestError('Description should have a minimum of 5 characters and a maximum of 150'))
    }
